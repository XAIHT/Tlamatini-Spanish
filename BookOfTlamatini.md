...
- **Parte IV — El Bestiario de Tlamatini**: referencia compacta de un renglón por agent para los 85 workflow agents.
...
Descarga el ZIP del release más reciente — **[Tlamatini v1.47.0](https://github.com/XAIHT/Tlamatini/releases/tag/v1.47.0)** — y descomprímelo (o usa una carpeta `Tlamatini_Release/` que alguien te haya pasado / que tú mismo hayas compilado — ve la Part VIII). Luego:
...
Una referencia compacta de los 84 tipos de workflow-agent. Los capítulos destacados de **Parametrizer** (§25) y **Gatewayer** (§26) están arriba; **Unrealer** tiene un capítulo extra completo en §57, **Blenderer** en §59 y **ESPHomer** en §60.
...
git tag -a v1.47.0 -m "Release 1.47.0: <one-liner>"   # annotated tag
...
Los tres scripts de build toman el tag de `git describe --tags` automáticamente. El artefacto final aterriza en `dist/Tlamatini_Release_v1.47.0/`, nombrado según la versión, para que el archivo que le entregues a un usuario sea inequívoco incluso antes de que lo descomprima.
...
- **Nombre de la carpeta de release** — `dist/Tlamatini_Release_v1.47.0/`.
...
| El "template project" | Vive *dentro* del repo del MCP y es específico del F407VG. | Es **este** repositorio independiente (`ESP32TemplateProject`), agnóstico de board y framework con editar un solo archivo. |
...
para ser su propio repositorio de GitHub (su casa natural, espejeando la del STM32, es
