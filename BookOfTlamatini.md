...
>
> **Estado comprobado:** el release actual es `v1.51.3s` (`212b0bd`); `HEAD` de `main` esta en el mismo commit, cero commits posteriores. La superficie viva contiene 88 workflow agents, 66 launchers `chat_agent_*`, 108 tools integradas de Multi-Turn, 105 tools del MCP stdio raíz, 29 skills, 200 migrations y 1,143 files tracked. La matriz source→docs y la separación release→desarrollo están en [`docs/estado-actual-v1.51.3s.md`](docs/estado-actual-v1.51.3s.md).
>
...
git tag -a v1.51.3s -m "Release 1.51.3s: <one-liner>"   # annotated tag
git push origin v1.51.3s
python build.py
python build_uninstaller.py
python build_installer.py
```

All three build scripts pick the release identity up from `git describe --tags` automatically. The final artefact lands in `dist/Tlamatini_Release_v1.51.3s/`, named for the version so the file you hand to a user is unambiguous before they even unzip it. The tag `v1.51.3s` resolves to `212b0bd`; the audited `HEAD` of `main` is that same commit, 0 commits later. Local working-tree changes do not move or rewrite the release tag.
...
- **Release folder name** — `dist/Tlamatini_Release_v1.51.3s/`.
- **Runtime surfaces** — the About dialog renders `Tlamatini v{{ version }}` (Django context processor); after a release build, the startup banner prints `--- [VERSION] Tlamatini 1.51.3s` to both the console and `tlamatini.log`; `GET /agent/version/` returns build-derived metadata such as `{"version":"1.51.3s","commit":"212b0bd","date":"…","source":"generated"}` for a build from audited `main`, or `commit:"212b0bd"` for the tagged tree. It is an **open** endpoint suitable for a health-check.
...
- **Release histórico `v1.50.4s` y linaje de seguridad — 2026-09-01** — Ese nombre histórico también apunta a `212b0bd`. El proceso Django frozen deja fuera `transformers` y Torch; `langchain.tools` se reemplaza por `langchain_core.tools`; el Python acarreado conserva y comprueba Torch CPU-only para Talker/Whisperer; el hook local impide que PyInstaller copie DLLs CUDA y el build rechaza una salida con Torch residual o un `pkg.zip` mayor de 2.8 GB. El linaje agregó cierre Ctrl+C acotado, build público desde clone limpio con `privacy_preflight()`, el contrato seguro de Deleter y el desenvolvimiento de trackers de Bing. La matriz actual —sin enlazar un archivo histórico eliminado— está en [`docs/estado-actual-v1.51.3s.md`](docs/estado-actual-v1.51.3s.md).

- **Contenido de `v1.51.3s` — español visible, composición de agents y pruebas propias — 2026-09-03/06** — El tag `v1.51.3s` y el `HEAD` de `main` son el mismo commit `212b0bd`: cero commits los separan. `views.py` superpone `agents_descriptions.es.md` sobre la tabla inglesa agent por agent, con fallback granular; `chat_agent_runtime.py` exporta `TLAMATINI_AGENTS_ROOT` para que Playwrighter encuentre a Shoter desde wrapped chat runs; la suite diaria añade barridos visibles de nueve diálogos ACP, tema, toggles bulk y un corpus reanudable de 1,000 preguntas españolas con control de respuestas rancias, idioma, registro técnico y gemelas con/sin acento. `Referenced Rephrase:` queda como sentinel de protocolo inglés, byte-stable y no duplicable. Los documentos de rediseño del update y de Memory MCP se registran como propuestas, no como implementación. El inventario actual es 1,143 files tracked, 250,620 líneas efectivas y 358,520 líneas físicas.
...
Descarga el ZIP del release más reciente — **[Tlamatini v1.51.3s](https://github.com/XAIHT/Tlamatini-Spanish/releases/tag/v1.51.3s)** — y descomprímelo (o usa una carpeta `Tlamatini_Release/` que alguien te haya pasado / que tú mismo hayas compilado — ve la Part VIII). Luego:
...
Los tres scripts de build toman el tag de `git describe --tags` automáticamente. El artefacto final aterriza en `dist/Tlamatini_Release_v1.51.3s/`, nombrado según la versión, para que el archivo que le entregues a un usuario sea inequívoco incluso antes de que lo descomprima.
...
- **Nombre de la carpeta de release** — `dist/Tlamatini_Release_v1.51.3s/`.
- **Superficies de runtime** — el diálogo About renderiza `Tlamatini v{{ version }}` (context processor de Django); el banner de arranque imprime `--- [VERSION] Tlamatini 1.51.3s` tanto en la consola como en `tlamatini.log`; `GET /agent/version/` devuelve metadata del build (por ejemplo `commit:"212b0bd"` desde `main` auditado o `commit:"212b0bd"` desde el tag), como un endpoint **abierto**, apto para un health-check.
...
- **ACPX ahora dice la verdad sobre sus pares — un exit code es UN BIT, y miente — 2026-09-07** — Un relevo de investigación entre varias CLIs se vino abajo mientras `acp_doctor` juraba que todos los agents externos estaban sanos. La causa: el doctor sólo corría `<cmd> --version`, y una CLI con la llave de API muerta, un archivo de configuración inválido o el saldo en cero **igual imprime su versión y sale con 0**. Peor todavía: un hijo que contestó *«la búsqueda fue bloqueada — no se concedió el permiso»* también salió con 0, así que la tool devolvió `ok: true`, la fila del Exec Report se pintó **verde**, y el LLM que orquestaba siguió adelante creyendo que traía investigación en la mano. Es exactamente la misma clase de entregable *silencioso, plausible y EQUIVOCADO* que el bug de las imágenes faltantes de PDFer y el veredicto del linter de LaTeXer: **un exit code es un bit, y un bit no alcanza para describir lo que hizo un coding agent**. Tres reparaciones, y ninguna se puede quitar. **(a)** `agent/acpx/child_health.py` — sólo stdlib, no importa `agent.*` — guarda la ÚNICA definición de «¿este hijo entregó?», con un vocabulario CERRADO de no-entrega (`PERMISSION_BLOCKED`, `WORKSPACE_NOT_TRUSTED`, `NO_CREDIT`, `USAGE_LIMIT`, `AUTH_FAILED`, `CONFIG_INVALID`, `UPSTREAM_ERROR`, `NO_OUTPUT`, `CHILD_ERROR`) y una tabla de firmas ordenada de lo más específico a lo más general. **(b)** `acp_doctor(deep=True)` manda un prompt real de una línea por el transport real de cada agent y NOMBRA la falla — opcional a propósito, porque gasta cuota del modelo, y cacheado diez minutos. **(c)** `acp_spawn` / `acp_send` devuelven `ok: false` con ese código cuando el hijo no produjo nada usable, conservando el `session_id` y la ruta del transcript para poder investigar y limpiar. Una respuesta larga y correcta que *menciona* un límite de tasa jamás se reclasifica como falla, y una respuesta corta y correcta jamás se confunde con silencio. **⚠️ EN ESTA EDICIÓN ESA TABLA HABLA LOS DOS IDIOMAS.** Los `code` son canal de máquina y se quedan en inglés byte-exactos; las RAZONES las lee Angela, así que van en español; y los PATRONES —que son la salida del CLI EXTERNO, que no es nuestro y puede hablar el idioma que se le antoje— conservan TODOS los del inglés y **suman** sus equivalentes en español, con y sin acento. Un `límite de uso` que no casara con `usage limit` haría que el clasificador dijera DELIVERED sobre una negativa y el Exec Report la pintara verde: el mismo bug, nada más que en español. Un patrón de más no cuesta nada; uno de menos cuesta una mentira. La misma pasada trae `agent/win_shim.py`, que centraliza el arranque de procesos en Windows y reescribe los shims `.cmd`/`.bat` a un argv directo de node, y le da a `config.json` la capacidad de reafinar `args`, `transport`, `prompt_arg_flag`, `prompt_subcommand_args`, los presupuestos de drenado y `spawn_returns_immediately` de un agent — no sólo `command` y `env` — así que reparar un par mal configurado es editar texto, no reconstruir. Cobertura: `agent/acpx/tests.py` creció a **126 tests** y `agent/test_win_shim.py` suma los suyos.

- **Dos bloques de código en una misma respuesta se destruían el archivo entre sí — 2026-09-06** — Los nombres de program y de snippet se arman como `<marca de tiempo UTC al SEGUNDO>_<nombre>`, así que dos bloques emitidos en la MISMA respuesta recibían el MISMO nombre — mientras que todos los lectores lo buscaban por NOMBRE con `.get()`. Ese par de filas quedaba inalcanzable **para siempre**: «Cargar en el canvas» reventaba con `MultipleObjectsReturned` — un 500 SIN ATRAPAR, porque `except LLMProgram.DoesNotExist` no lo cacha — y guardar los archivos desde la base fallaba igual **para las dos gemelas**, así que ninguno de los dos se escribía jamás al disco. El contenido nunca se perdió; nada más se volvió inalcanzable por su propio nombre. El arreglo es de los DOS lados y ninguna mitad se puede quitar: **(1)** el nombre se hace único al guardar (`services/response_parser._uniquify_name`, que devuelve `<nombre>_2` / `_3` … y es FAIL-OPEN — cualquier error de base cae al nombre original, porque no poder desambiguar jamás debe impedir que el código se guarde), y **(2)** **los cuatro lectores** pasaron a `filter(...).first()`: `views.load_canvas_view`, `consumers.get_program_by_name`, `services/filesystem.save_files_from_db` y `rag/interface.get_program_by_name`. La migración `0200` repara las bases escritas por un build viejo, y es **NO DESTRUCTIVA por diseño**: no borra nada, la fila de id más bajo conserva su nombre, cada gemela posterior se renombra, y es idempotente y fail-open — un migrate que aborta dejaría a la usuaria sin sus agents nuevos, que es mucho peor que un duplicado sin reparar.
