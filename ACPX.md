...
> **Estado actual (Tlamatini-Spanish — `v1.51.3s` es el release vigente en `1339fc7`; el `HEAD` auditado y `origin/main` están en `272d6ac`, cinco commits posteriores).** No se citan commits del árbol inglés porque no existen en este repository. Este archivo conserva el walkthrough original de ACPX/Skills y sus cifras por sección son un **snapshot histórico de Phase 1** (5 `@tool`s ACPX, 20 seed skills, 14 `agent_id`, 57 agents visuales). Las cifras actuales y autoritativas son **88 tipos de agent visual**, **108 tools integradas de Multi-Turn** (20 core + 66 `chat_agent_*` + 12 ACPX/Skill + 10 supervisores External-MCP), **29 skills** y una superficie ACPX/Skill de **12 tools para el LLM** (`acp_doctor`, `list_acp_agents`, `acp_spawn`, `acp_send`, `acp_send_and_wait`, `acp_kill`, `acp_transcript`, `acp_session_status`, `acp_list_sessions`, `acp_relay`, `list_skills`, `invoke_skill`). Los remotos dinámicos `ext__*` se cuentan por separado. `DEFAULT_ACP_AGENTS` conserva **14** entradas (§3.5). Para la superficie viva consulta `CLAUDE.md` y `docs/claude/acpx.md`; las cifras antiguas debajo no son estado actual.
