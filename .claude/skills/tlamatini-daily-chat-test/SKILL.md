...
## v1.51.3s current regression set

Verify tag `v1.51.3s` resolves to `212b0bd`; separately report the `HEAD` of `main` (that same commit, 0 commits later) and dirty working-tree changes. Run `python manage.py test agent.test_pareo_con_el_ingles` (13 tests, ~7 s) — it compares surfaces against the English tree, pins the +1 migration offset, and requires every commit hash cited in the docs to resolve here, declare it is English-tree, or sit in `HUERFANOS_CONOCIDOS`; it SKIPS honestly when that tree is not cloned. Assert the current 88/66/108/29/200 source counts plus the 1,143-file tracked inventory. Run the lean-process guard: Django frozen excludes `transformers` and Torch, the carried Python still passes `_probe_cpu_torch()` with CPU-only Torch for Talker/Whisperer, the priority hook cannot collect CUDA DLLs, `verify_frozen_torch_absent()` rejects residue, and `pkg.zip` is capped at 2.8 decimal GB. Run the genuine Ctrl+C visible proof plus `test_ctrl_c_shutdown`; test the clean-clone/no-target and private-evidence/refusal branches of the public builder; prove Bing tracker unwrapping after HTML unescape; and test Deleter's `target_path`/`files_to_delete` split, explicit directory-delete opt-in, protected-root refusals, reason logging, and downstream triggering. Also validate the agent-by-agent Spanish-description overlay/fallback, the `TLAMATINI_AGENTS_ROOT` Playwrighter→Shoter path, every new visible dialog/theme/bulk harness, the resumable 1,000-question Spanish harness guards, and the exact single `Referenced Rephrase:` sentinel.
