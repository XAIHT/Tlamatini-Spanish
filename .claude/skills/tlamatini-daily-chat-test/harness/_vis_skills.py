...
for ln in open(".creds.env", encoding="utf-8"):
    if "=" in ln and not ln.strip().startswith("#"):
        k, v = ln.split("=", 1)
        if k.strip() == "TLAMATINI_USER": u = v.strip()
        if k.strip() == "TLAMATINI_PASS": c = v.strip()
...
with sync_playwright() as pw:
    ...
    page.fill("#id_username", u); page.fill("#id_password", c)
    ...
    time.sleep(2.0)
    page.click("#skills-menu-button"); time.sleep(0.5)
    page.click("#configure-skills"); time.sleep(1.5)
    page.bring_to_front(); time.sleep(0.4)
