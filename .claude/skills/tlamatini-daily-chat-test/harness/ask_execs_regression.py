...
SEL = {
    ...
    # ⚠️ "Continu" a secas, NO "Continue": el botón de la edición española dice
    # "Continuar" (agent_page_dialogs.js::makeDialogButtons). has-text() hace
    # match por SUBCADENA, así que "Continu" pega con las DOS formas y el
    # harness sirve igual contra un build viejo en inglés. Con "Continue"
    # completo, este selector no encontraba nada y el escenario se colgaba
    # hasta el timeout.
    "confirm_continue": '.ui-dialog-buttonpane button:has-text("Continu")',
    # Results
...
def s1_proceed_runs_the_tool(page) -> None:
    ...
    check("S1", "no red 'Ejecución interrumpida' banner",
...
def s2_deny_halts_the_chain(page) -> None:
    ...
    check("S2", "the red 'Ejecución interrumpida' banner is shown", banner,
