...
def _contains_not_ready(texts: List[str], prev_count: int) -> bool:
    ...
    for t in _fresh_texts(texts, prev_count):
        tt = (t or "").strip()
        if tt and C.is_not_ready(tt):   # accent-folded: matches ES + EN banners
            return True
    return False
...
def _filter_answer(texts: List[str], prev_count: int) -> str:
    ...
    for t in fresh:
        ...
        if not tt:
            continue
        if C.is_busy_banner(tt):        # accent-folded: matches ES + EN banners
            continue
        kept.append(tt)
    if kept:
        return kept[-1]
    ...
    for t in reversed(texts):
        tt = (t or "").strip()
        if tt and not C.is_busy_banner(tt):   # accent-folded: ES + EN banners
            return tt
    return ""
