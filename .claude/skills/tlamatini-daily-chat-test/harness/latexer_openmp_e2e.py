...
* Full-desktop screenshots are taken by Tlamatini's own SHOTER agent
  (``shoter_foto.toma_foto`` - the Spanish tree's launcher) - never PIL.ImageGrab.
...
# SHOTER, el agent de Tlamatini. Ver shoter_foto.py. (PIL.ImageGrab esta PROHIBIDO.)
...
# Spanish edition: this harness lives in the Tlamatini-Spanish tree, so its
# scratch work goes under THAT tree's Temp (Temp/Templates policy, 2026-06-02).
APP_ROOT = os.environ.get("TLAMATINI_APP_ROOT", r"C:\Development\Tlamatini-Spanish")
...
# (Override with TLAMATINI_OPENMP_CU when the Spanish edition is installed under
# a different root than the English one.)
ORIGINAL_CU = os.environ.get(
    "TLAMATINI_OPENMP_CU",
    r"C:\Tlamatini\context_files\CompleteOpenMPImplementation.cu",
)
