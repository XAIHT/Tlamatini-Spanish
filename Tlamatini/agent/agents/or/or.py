...
# OR Agent - Deterministic OR gate logic
# Monitors two source log files for specific patterns.
# Raises target agents if ANY of the patterns are detected.
...
def get_agent_log_path(agent_name: str) -> str:
    ...
    # Log files use full agent name with cardinal (e.g., monitor_log_1 -> monitor_log_1.log)
...
def main():
    ...
    try:
        ...
        if not source_1 and not source_2:
            logging.warning("⚠️ No source agents configured. OR Agent handles 0 inputs as 'False' (never triggers).")
        
        logging.info("🔥 OR AGENT STARTED")
        if source_1:
        ...
        while True:
            ...
            if source_1:
                ...
                if found_1:
                    triggered = True
                    trigger_reason += f"[Input 1: {source_1} - {line_1}] "
            ...
            if source_2:
                ...
                if found_2:
                    triggered = True
                    trigger_reason += f"[Input 2: {source_2} - {line_2}] "

            if triggered:
                logging.info(f"🚨 OR GATE TRIGGERED, EVENT DETECTED: {trigger_reason}")
            ...
            if poll_count % 10 == 0:
                ...
                if source_1:
                    ...
                    src_status.append(f"{source_1}:{'✓' if exists_1 else '?'}")
                if source_2:
                    log_path_2 = get_agent_log_path(source_2)
                    exists_2 = os.path.exists(log_path_2)
                    src_status.append(f"{source_2}:{'✓' if exists_2 else '?'}")
