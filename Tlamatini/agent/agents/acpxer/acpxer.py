...
def start_agent(agent_name: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            logging.error(f"⚠️ No se pudo escribir el archivo PID de {agent_name}: {pid_err}")
...
def append_transcript_event(direction: str, text: str, raw: str = "") -> None:
    ...
    try:
        ...
        logging.error(f"❌ No se pudo escribir la transcripción: {e}")
...
def run_acpx_session(config: dict) -> dict:
    ...
    if not task:
        logging.error("❌ No hay tarea configurada (config.task está vacío)")
        return {
    ...
    try:
        ...
        logging.error(f"❌ Falló el arranque: {e}")
        return {
            "ok": False, "session_id": session_id, "agent_id": agent_id,
            "transport": transport, "transcript_path": transcript_path,
    ...
    try:
        ...
        logging.error(f"❌ Falló el envío: {e}")
        try:
            process.terminate()
        except Exception:
            pass
    ...
    try:
        if process.poll() is None:
            ...
            try:
                ...
                process.kill()
        logging.info(f"🛑 Sesión terminada: el PID era {process.pid}")
    except Exception as e:
        logging.error(f"⚠️ Aviso al terminar: {e}")
