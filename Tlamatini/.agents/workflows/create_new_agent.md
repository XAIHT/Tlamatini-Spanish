...
5b. **⚠️ `status:` IS LOAD-BEARING — it decides the Exec-Report colour.** `agent/agent_verdict.py` parses this block and **the value you write here OUTRANKS the process exit code**. The vocabulary is CLOSED: select exactly one literal already present in `DIAGNOSTIC_COMPLETED_STATUSES`, `WORK_COMPLETED_STATUSES`, `WORK_DEGRADED_STATUSES`, `WORK_NOT_DONE_STATUSES`, or `AGENT_ERROR_STATUSES`; if a genuinely new semantic token is required, add it to exactly one shared set and extend the guard test. Diagnostics that completed (`invalid`, `findings`, `no_matches`, `listed`) and intact work (`ok`, `created`, `sent`) are green; compromised deliverables (`compiled_with_errors`, `tokens_only`, `partial`), work not done (`refused`, `not_found`, `timeout`), and malfunctions (`error`, `failed`) are red. **Never place a numeric return code in `status:`** — emit `returncode: <int>`, `success: <bool>`, and a semantic status token. A read-only agent must exit **0** even when it finds problems; only an action that failed to deliver may exit non-zero. `agent/test_status_vocabulary.py` rejects unknown literals and numeric-status interpolation. Full rule order: `docs/claude/exec-report.md` → *Success/failure classification*.
...
**⚠️ The row's SUCCESS/FAILED colour comes from your agent's semantic `status:`, not from its exit code.** `agent/agent_verdict.py` gives the `INI_SECTION` self-report precedence, so emit one token from its five-set closed vocabulary, make a completed read-only diagnostic exit 0 even when it finds problems, and keep degraded outputs distinct from intact completion. If a row is wrong, fix the emitted token or its membership in the ONE shared vocabulary — never add a special case to `mcp_agent.py`, re-inline a set, or write `status: <numeric return code>`. Contract: `docs/claude/exec-report.md`.
...
[ ] 1. agent/agents/<agent_name>/<agent_name>.py + config.yaml
    ...
    [ ] TRUTHFUL semantic `status:` from exactly one of agent_verdict.py's five sets;
        ...
        numeric return code in status; emit returncode + success separately, and run
        agent.test_status_vocabulary
