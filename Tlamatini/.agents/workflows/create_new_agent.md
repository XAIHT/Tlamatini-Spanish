...
5b. **⚠️ `status:` IS LOAD-BEARING (since v1.48.2) — it decides the Exec-Report colour.** `agent/agent_verdict.py` parses this very block and **the value you write here OUTRANKS your process exit code**. So write it truthfully, and use the shared vocabulary: a **read-only diagnostic that ran to completion** reports `validated` / `invalid` / `findings` / `matches` / `no_matches` / `listed` / `analyzed` … and is scored **SUCCESS** even when the finding is adverse (the finding is the DELIVERABLE); work that did **not** happen reports `refused` / `not_found` / `not_unique` / `engine_unavailable` / `timeout` … and is scored **FAILED**; a genuine malfunction reports `error` / `failed`. **Corollary for `sys.exit()`**: a read-only agent must exit **0** even when it finds problems — NEVER tie the exit code to how clean the user's input was (that exact bug made LaTeXer's working linter render red; see `docs/claude/recent-fixes.md` 2026-08-06). Only an action that failed to do the requested work may exit non-zero. Full vocabulary + rule order: `docs/claude/exec-report.md` → *Success/failure classification*.
...
**⚠️ The row's SUCCESS/FAILED colour comes from your agent's `status:`, not from its exit code (v1.48.2).** `agent/agent_verdict.py` parses the `INI_SECTION` self-report and gives it **precedence over the process exit code**, so the one thing you must get right is Step 1b.5b: emit a truthful `status:` from the shared vocabulary, and make a **read-only diagnostic exit 0 even when it finds problems**. If your agent's row is coloured wrong, fix the `status:` it emits — do **not** add a special case to `mcp_agent.py`, and do **not** re-inline a second copy of `DIAGNOSTIC_COMPLETED_STATUSES` (one definition only, in `agent_verdict.py`). Contract: `docs/claude/exec-report.md`.
...
[ ] 1. agent/agents/<agent_name>/<agent_name>.py + config.yaml
    ...
    [ ] TRUTHFUL `status:` in that block — it OUTRANKS the exit code and decides the
        Exec-Report colour (agent_verdict.py, v1.48.2). Read-only diagnostic that finds
        problems => diagnostic status + `sys.exit(0)`; work that did NOT happen =>
        refused/not_found/engine_unavailable; malfunction => error/failed
