...
14. **The Exec Report capture point in `mcp_agent.py` is `_invoke_tool()`, not the chain layer.** Capture is unconditional (ignores the per-request flag). The flag only gates rendering. This separation is deliberate to prevent whitelist-style bugs from silently hiding data again. **(2026-06-07) Capture covers EVERY wrapped `chat_agent_*` automatically** via `_resolve_exec_report_spec` (the curated `_EXEC_REPORT_TOOLS` map is only an optional styling/merge refinement now) — observational/output and read-only agents included, plus newly-created ones. Only `_MANAGEMENT_TOOLS` polling helpers and direct read-only @tools (`googler`) are excluded. **(2026-08-06, v1.48.2) The row's SUCCESS/FAILED verdict is now decided by `agent/agent_verdict.py`**, a deterministic parser + ordered rule table over the agent's own `INI_SECTION` self-report — and **that self-report OUTRANKS the process exit code**. So a read-only diagnostic reporting an adverse finding (`invalid`, `findings`, `no_matches`, …) renders **green**, while `refused` / `not_found` / `engine_unavailable` render **red**. Emit a truthful `status:` from the shared vocabulary and never tie a read-only agent's exit code to how clean the user's input was. The vocabulary has ONE definition (`agent_verdict.DIAGNOSTIC_COMPLETED_STATUSES`) — do not re-inline a copy. See `docs/claude/exec-report.md` → *Success/failure classification*.
