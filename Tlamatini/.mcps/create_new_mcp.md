...
>
> **External MCPs** is a config-driven *universal MCP client* (`agent/external_mcp_manager.py`, catalog `agent/external_mcps.json`) that connects Tlamatini to **ANY third-party MCP server declared in a JSON file** (the standard `mcpServers` shape), over four transports — **stdio**, **streamable-http**, legacy **sse**, and **websocket** — with up to **5 active** at once. Each remote tool is auto-wrapped as **`ext__<server>__<tool>`**. The LLM drives the subsystem with **10 supervisor tools**: status / reconnect / doctor / runtime-status / runtime-install / list-tools / call / import / activate / wait. `agent/runtime_provisioner.py` resolves and can atomically install missing Node/npm/npx/pnpm or uv/uvx under `%LOCALAPPDATA%\Tlamatini\runtimes`, with no admin and no system-PATH change. The user adds servers through **External ▸ MCPs** (search/import/activation plus runtime readiness and **Install now**), or the LLM calls `external_mcp_import` → `external_mcp_set_active` → `external_mcp_wait`.
>
...
>
> Every installation also carries code-owned `memory` and `sequential-thinking` defaults from `agent/external_mcp_defaults.py`, both inactive. Existing edits win and deleted defaults are tombstoned. A new shipped default must be declared in code and seeded by `load_catalog()`; changing only `external_mcps.json` reaches fresh installs but not upgraded users because the catalog is preserved state. Public builds must remain secret-free and are generated separately from keyed/private catalogs. Full architecture: `docs/external_mcp_bulletproof_architecture.md`; current contract: `docs/claude/recent-fixes.md` (2026-08-15).
>
...
18. **"External MCPs" is a SEPARATE subsystem — not the `Mcp` rows, not the inline pool-agent clients (see the boxed note near the top).** If the user wants the tools of an existing third-party MCP server, use `external_mcp_manager.py` + `external_mcp_defaults.py` + `runtime_provisioner.py` + the sanitized/preserved catalog and **External ▸ MCPs**, NOT a new `@tool` / `Mcp` row / context provider. Remote tools become `ext__<server>__<tool>`; ten supervisors manage ≤5 active servers over four transports. New shipped defaults must be code-seeded, inactive, tombstone-aware, public-build safe, and covered by runtime/default-catalog tests.
