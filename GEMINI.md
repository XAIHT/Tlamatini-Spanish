...
### 1.5 Current Release (`v1.50.0s` target — Spanish edition)
- **`v1.50.0s` is the package/documentation target for this Spanish edition**; the newest annotated tag actually in THIS tree is `v1.49.1s` (`c02056e7`). Runtime identity stays git-tag-derived. The trailing **`s` is the edition letter** — kept in every human-readable surface, stripped wherever a version becomes numbers (`agent/version.py::strip_edition_suffix`); see `VERSIONING.md` → *The Spanish edition letter*. Do not quote the English tree's commit hashes: they do not exist in this repository.
