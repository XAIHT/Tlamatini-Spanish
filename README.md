...
<p align="center">
  ...
  <a href="https://github.com/XAIHT/Tlamatini-Spanish/releases/tag/v1.51.3s"><img src="https://img.shields.io/badge/VERSION-v1.51.3s-1E90FF?style=for-the-badge&labelColor=2D2D2D" alt="Versión"/></a>
...
## Estado actual — `v1.51.3s`

El release/tag vigente `v1.51.3s` apunta a `1339fc7`. El `HEAD` auditado y `origin/main` apuntan a `272d6ac`, **cinco commits posteriores al tag**; esa línea de desarrollo conserva la identidad visible `1.51.3s`, pero no se presenta como contenido ya publicado del tag. La superficie derivada del source es **88 workflow agents**, **66 launchers `chat_agent_*`**, **108 tools integradas de Multi-Turn**, **105 tools del MCP stdio raíz**, **29 skills** y **198 migrations**. El inventario actual contiene **1,143 files tracked**, **250,620 líneas efectivas** y **358,520 líneas físicas**. La matriz de evidencia vive en [`docs/estado-actual-v1.51.3s.md`](docs/estado-actual-v1.51.3s.md).

La línea más reciente separa deliberadamente dos intérpretes: el proceso Django frozen excluye `transformers` y Torch para arrancar limpio y ligero, mientras el Python acarreado conserva Torch CPU-only para **Talker** y **Whisperer**. Googler desenvuelve trackers de Bing; el build público puede partir de un clone limpio y aplica un `privacy_preflight`; el cierre por Ctrl+C es acotado y no ejecuta cleanup complejo dentro del signal handler; y Deleter distingue estrictamente `target_path` (directorio de trabajo) de `files_to_delete` (objetivos).

Después del tag, `agent/views.py` superpone `agents_descriptions.es.md` sobre la tabla inglesa agent por agent y conserva fallback granular; `chat_agent_runtime.py` entrega `TLAMATINI_AGENTS_ROOT` a los wrapped agents para que Playwrighter encuentre a Shoter desde Multi-Turn; y la suite visible incorpora barridos de diálogos/tema/toggles más un corpus reanudable de 1,000 preguntas españolas. El sentinel interno `Referenced Rephrase:` permanece en inglés y byte-stable por NEPANTLA, se filtra del historial y no se duplica.

También después del tag, **ACPX dice la verdad sobre sus pares**. Un relevo de investigación entre varias CLIs se venía abajo mientras `acp_doctor` reportaba a todos los agents externos como sanos — porque el doctor sólo corría `<cmd> --version`, y una CLI con la llave de API muerta, un archivo de configuración inválido o el saldo en cero igual imprime su versión y sale con 0. Peor: un hijo que contestaba *«la búsqueda fue bloqueada — no se concedió el permiso»* también salía con 0, así que la tool devolvía `ok: true` y la fila del Exec Report se pintaba VERDE sobre trabajo que nunca ocurrió. Tres reparaciones: `agent/acpx/child_health.py` (sólo stdlib) guarda la única definición de «¿este hijo entregó?» con un vocabulario cerrado de no-entrega (`PERMISSION_BLOCKED`, `AUTH_FAILED`, `CONFIG_INVALID`, `NO_CREDIT`, `USAGE_LIMIT`, `UPSTREAM_ERROR`, `NO_OUTPUT`, `WORKSPACE_NOT_TRUSTED`, `CHILD_ERROR`); `acp_doctor(deep=True)` manda un prompt real de una línea por el transport real de cada agent y nombra la falla (opcional, porque gasta cuota del modelo, y se cachea diez minutos); y `acp_spawn` / `acp_send` devuelven `ok: false` con ese código cuando el hijo no produjo nada usable, conservando el session id y la ruta del transcript para poder investigar y limpiar. Una respuesta larga y correcta jamás se reclasifica como falla, y una respuesta corta y correcta jamás se confunde con silencio. **En esta edición esa tabla trae los patrones en los DOS idiomas**: se conservan todos los del inglés y se agregan sus equivalentes en español, con y sin acento — un `límite de uso` que no casara con `usage limit` volvería a pintar de verde una negativa. Además `config.json` ya puede reafinar `args`, `transport`, `prompt_arg_flag`, `prompt_subcommand_args`, los presupuestos de drenado y `spawn_returns_immediately` de un agent — no sólo `command` y `env` — así que reparar un par mal configurado es editar texto y no reconstruir. `agent/win_shim.py` centraliza el arranque de procesos en Windows (los shims `.cmd`/`.bat` se reescriben a un argv directo de node).

Y se cerraron las **colisiones de nombre de program/snippet**: dos bloques de código de la misma respuesta compartían la llave `<marca_de_tiempo>_<nombre>` (resolución de un segundo), así que `.get()` lanzaba `MultipleObjectsReturned` y las dos filas gemelas quedaban inalcanzables — «Cargar en el canvas» fallaba con un 500 sin atrapar y ninguno de los dos archivos se escribía al disco. El arreglo es de los dos lados y ninguna mitad se puede quitar: el nombre se hace único al guardar (`services/response_parser._uniquify_name`) y **los cuatro lectores** (`views.load_canvas_view`, `consumers.get_program_by_name`, `services/filesystem.save_files_from_db` y `rag/interface.get_program_by_name`) pasaron a `filter(...).first()`. La migración `0200` repara, sin borrar nada, las bases escritas por un build viejo.

Los archivos `10000xRedesignOfUpdatingMechanicsOnTlamatini.md`, `DesignOfIncludingMemoryMCPS.txt` y `MCPMemoriesFlowCreation.flw` son propuestas de diseño; no constituyen por sí solos features de runtime implementadas.
...
- **PDFer** — el **compositor de documentos**: convierte la propia respuesta de Tlamatini, algo de Markdown/HTML, texto plano, una carpeta de imágenes o varios PDFs existentes en UN solo PDF con estilo — con portada, tablas reales, números de página y un índice opcional. Es el lado de ESCRITURA de la familia documental (File-Extractor / File-Interpreter *leen* documentos; PDFer los *escribe*). **No necesita instalación** — todos los motores que usa ya vienen dentro de Tlamatini. Modos: `auto` (olfatea el contenido por ti) / `markdown` / `html` / `text` / `images` (uno por página, ajustado, o en cuadrícula) / `mixed` (prosa + figuras incrustadas) / `merge` / `info` / `validate`. **Lee lo que escribiste y decide cómo debe verse el documento, antes de renderizar nada.** Algo sobre ciencia o tecnología sale en página casi negra con letra blanca, degradados de cian a violeta y un campo de estrellas generado; un documento con abstract y citas sale blanco puro, negro y justificado, compuesto en Palatino como una revista tipografía un artículo; un contrato sale liso, en Times, sin una sola decoración. Veinte tratamientos en total, escogidos por el vocabulario, la estructura y el tono de tu texto — y le puedes ganar con **`nuance`** (`science`, `paper`, `legal`, `receta`… cualquier alias, en inglés o en español) o darle un solo **`predominant_color`** y ver cómo *toda* la paleta — títulos, filetes, encabezados de tabla, barras de callout, el degradado de la portada, el pie de página — se deriva de ese único color. **Sus tablas no se pueden encimar.** El ancho de cada columna se resuelve con las métricas reales de la fuente antes de colocar una letra, así que una ruta larga de Windows o una URL se acomoda dentro de su propia celda en vez de imprimirse encima de la siguiente; y una vez escrito el archivo, PDFer lo vuelve a abrir y mide las letras reales de la página buscando texto encimado, tinta que se sale de la hoja, páginas en blanco y el contraste real, para poder decirte que el layout quedó *verificado* y no supuesto. **Dibuja sus propias ilustraciones** — lavados de degradado, campos de estrellas, trazos de circuito, ondas, formas orgánicas suaves — en los colores del propio documento, sin archivos de imagen y sin descargas, y sólo donde el contenido lo hace seguro: un contrato, una tabla de dosis y un estado financiero no reciben ninguna. Opcionalmente deja que un modelo de Ollama ordene antes el texto en Markdown limpio, o que te haga la dirección de arte de los colores — los dos apagados por omisión, y ninguno puede perderte el documento. Los PDFs aterrizan en **Documents/TlamatiniPDF** con un nombre a prueba de colisiones, y un preflight a prueba de fallos prefiere negarse antes que escribir un archivo vacío o equivocado.
