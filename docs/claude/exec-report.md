...
- **ACPX rows became truthful in v1.51.2s (2026-09-07)** without any change here. An ACP child that refused, could not authenticate, had no credit or printed nothing used to exit **0** and land in the Exec Report as a GREEN row; `agent/acpx/child_health.py` now classifies the child's real output and `tools._ok_unless_blocked` emits `{"ok": false, "code": "PERMISSION_BLOCKED" | "AUTH_FAILED" | …}`, which this same legacy `{"ok": false}` branch renders red. The lesson generalises: **an exit code is one bit and cannot describe what a sub-agent did** — the same reason `agent_verdict.py` exists for pool agents. See `docs/claude/acpx.md` → *The delivery verdict*.
