...
**`1.51.3s` es el release actual del package y la documentación de esta edición en español**; el tag de ESTE árbol es `v1.51.3s` en `212b0bd`. Ese commit es tambien el `HEAD` de `main`: cero commits posteriores: reporta release y estado de desarrollo por separado. La **`s` es la letra de edición** que diferencia este build del árbol inglés `v1.51.3`: se conserva en toda superficie humana y `agent/version.py::strip_edition_suffix` la elimina donde la versión debe volverse numérica; véase `VERSIONING.md`, fijado por `agent/test_edition_version_suffix.py`. No cites hashes del árbol inglés: no existen aquí. La verdad derivada del source es **88 workflow agents**, **66 launchers `chat_agent_*`**, **108 tools integradas de Multi-Turn** (20 core + 66 wrapped + 12 ACPX/Skill + 10 supervisores External-MCP), **105 tools del MCP stdio raíz**, **29 skills**, **200 migrations** y **1,143 files tracked**. No escribas estas cifras por intuición: `agent/test_self_knowledge_is_current.py` las deriva y falla si `agent/Tlamatini.md` se desvía. **El pareo con el árbol inglés también es una prueba, no un barrido a mano:** `agent/test_pareo_con_el_ingles.py` compara superficies (agents, skills, frontend, keys de `config.json`, launchers `chat_agent_*`, el AST de cada `.py` compartido), fija el desfase +1 de migrations, y exige que todo hash citado o resuelva aquí, o declare que es del árbol inglés, o esté en la lista de huérfanos conocidos con su motivo. Los renames traducidos a propósito se declaran en `PAREADOS`; lo que sólo debe existir allá, en `SOLO_DEL_ARBOL_INGLES`. Si el árbol inglés no está clonado la guarda **se salta** (apúntala a otra ruta con `TLAMATINI_ARBOL_INGLES`): ponerse roja porque falta OTRO repository sólo enseña a ignorar el color.

**Cambios que el tag `v1.51.3s` ya contiene y que son source real:** descripciones españolas superpuestas agent por agent con fallback inglés; `TLAMATINI_AGENTS_ROOT` dentro del entorno de wrapped agents para que Playwrighter encuentre a Shoter desde chat; harnesses visibles de diálogos/tema/toggles y 1,000 preguntas en español; `Referenced Rephrase:` como sentinel machine byte-stable y no duplicable; fields promovidos nuevos en Parametrizer; el Atelier de PDFer (nuance / tipografía / layout, 10 módulos hermanos, migration `0199`); **ACPX que dice la verdad sobre sus pares** (`agent/acpx/child_health.py` con el vocabulario cerrado de no-entrega, `acp_doctor(deep=True)`, `agent/win_shim.py`, y overrides de spec desde `config.json`) — con la tabla de patrones EN LOS DOS IDIOMAS en esta edición, porque un `límite de uso` que no casara con `usage limit` volvería a pintar de verde una negativa; y el cierre de las colisiones de nombre de program/snippet (`_uniquify_name` al guardar + los CUATRO lectores en `filter(...).first()` + migration `0200`). Los documentos de rediseño de update y Memory MCP son propuesta, no runtime implementado.
