...
> **The Book of Tlamatini** — a step-by-step guide to running, using, and mastering a locally-deployed AI developer assistant with RAG, Multi-Turn tool orchestration, ACPX external-CLI delegation, an Unreal MCP client for driving Unreal Engine 5 from chat or canvas, a visual workflow designer, 85 drag-and-drop agent types, and a backend Flow Compiler that turns the live canvas — or a chat-generated tool-call log — into a registry-validated, secret-redacted, source-and-frozen-portable workflow.
>
...
There is a quiet economic argument hiding inside this software, and it is worth saying out loud before you install a thing. A frontier subscription — GPT-5.4, Claude Opus, and their kin — asks roughly **$200 every month** to talk to one model. Tlamatini turns that arithmetic on its head. **The app is free** — you never pay us; the single bill is **Ollama Pro, about $200 a *year*** (paid to Ollama), and around that one cloud connection she wraps **86 agent types and 75+ tools** that run on *your* machine. Comparable power, for roughly one-twelfth of the bill. That is why this chapter opens the book.
...
- **Part IV — The Tlamatini Bestiary**: compact one-row-per-agent reference for all 85 workflow agents.
...
Download the latest release ZIP — **[Tlamatini v1.47.0](https://github.com/XAIHT/Tlamatini/releases/tag/v1.47.0)** — and unzip it (or use a `Tlamatini_Release/` folder somebody handed you / you built — see Part VIII). Then:
...
All three build scripts pick the tag up from `git describe --tags` automatically. The final artefact lands in `dist/Tlamatini_Release_v1.47.0/`, named for the version so the file you hand to a user is unambiguous before they even unzip it.
...
- **Release folder name** — `dist/Tlamatini_Release_v1.47.0/`.
- **Runtime surfaces** — the About dialog renders `Tlamatini v{{ version }}` (Django context processor); the startup banner prints `--- [VERSION] Tlamatini 1.47.0` to both the console and `tlamatini.log`; `GET /agent/version/` returns `{"version":"1.47.0","commit":"abc1234","date":"…","source":"generated"}` as an **open** endpoint suitable for a health-check.
