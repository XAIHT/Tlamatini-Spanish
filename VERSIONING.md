...
2. **Single source of truth**: a **git tag** of the form `v1.50.2s`.
...
4. **Three injection points**, all computed automatically at build time:
   ...
   - The release folder name (`dist/Tlamatini_Release_v1.50.2s/`)
...
**Tlamatini-Spanish tags its releases with a trailing edition letter — `v1.50.2s` —
so a Spanish build is distinguishable at a glance from the English tree's `v1.50.2`.**
...
| Surface | Shows |
|---|---|
| About dialog, startup banner, `GET /agent/version/` | `1.50.2s` — the letter is kept |
| `.exe` → Properties → Details → ProductVersion (string) | `1.50.2s` — the letter is kept |
...
purpose and returns `None` for `1.50.2s`.
...
build.py / build_installer.py / build_uninstaller.py
   ...
   ├─►  dist/Tlamatini_Release_v1.50.2s/      (release folder name)
   └─►  Runtime surfaces:
            ├── About dialog:   "Tlamatini v1.50.2s"
            ├── Console banner: "--- [VERSION] Tlamatini 1.50.2s"
...
|---|---|---|---|
| 1 | **`--version X.Y.Z` CLI flag** | `python build.py --version 1.50.2s` | `1.50.2s` |
| 2 | **`$env:TLAMATINI_VERSION`** | `$env:TLAMATINI_VERSION = "1.50.2s"; python build.py` | `1.50.2s` |
| 3 | **`git describe --tags --abbrev=0 --match 'v[0-9]*'`** | `git tag -a v1.50.2s -m "..."; python build.py` | always the bare base tag → `1.50.2s` (distance / dirty state never appear in the version string) |
...
The commands below show the current `v1.50.2s` release as a concrete example. That tag already exists: **do not rerun these commands, move it, or recreate it**. Substitute the next deliberate version when preparing a future release.

```powershell
git tag -a v1.50.2s -m "Release 1.50.2s: <one-line summary>"
```
...
All three scripts pick up `v1.50.2s` automatically (precedence #3 — Git finds the newest reachable annotated tag). This remains true when `HEAD` is a later untagged commit, because Tlamatini deliberately emits the bare base tag without a distance or dirty suffix.

You'll see this in each script's output:
```
Tlamatini version : 1.50.2s
...
The final artefact is **`dist/Tlamatini_Release_v1.50.2s/`** — zip and distribute.
...
- **About dialog**: `Tlamatini v1.50.2s`
- **Right-click `Tlamatini.exe` → Properties → Details**: ProductVersion = `1.50.2s`
- **Console banner on startup**: `--- [VERSION] Tlamatini 1.50.2s`
- **`curl http://localhost:8000/agent/version/`** (after login or with anonymous access since it's open):
  ```json
  {"version":"1.50.2s","build":"1.50.2s","commit":"abc1234","date":"2026-08-23T12:00:00Z","source":"generated"}
  ```
...
TAG IT                git tag -a v1.50.2s -m "Release 1.50.2s"
                      git push origin v1.50.2s
...
WHERE IT LANDS        dist/Tlamatini_Release_v1.50.2s/
                      About dialog : "Tlamatini v1.50.2s"
                      Exe → Properties → Details : ProductVersion = 1.50.2s
                      curl /agent/version/ : {"version":"1.50.2s", …}
                      Console banner : --- [VERSION] Tlamatini 1.50.2s
