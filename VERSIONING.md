...
2. **Single source of truth**: a **git tag** of the form `v1.51.3s`.
...
| Surface | Shows |
|---|---|
| About dialog, startup banner, `GET /agent/version/` | `1.51.3s` — the letter is kept |
| `.exe` → Properties → Details → ProductVersion (string) | `1.51.3s` — the letter is kept |
...
purpose and returns `None` for `1.51.3s`.
...
build.py / build_installer.py / build_uninstaller.py
   ...
   ├─►  dist/Tlamatini_Release_v1.51.3s/      (release folder name)
   └─►  Runtime surfaces:
            ├── About dialog:   "Tlamatini v1.51.3s"
            ├── Console banner: "--- [VERSION] Tlamatini 1.51.3s"
...
|---|---|---|---|
| 1 | **`--version X.Y.Z` CLI flag** | `python build.py --version 1.51.3s` | `1.51.3s` |
| 2 | **`$env:TLAMATINI_VERSION`** | `$env:TLAMATINI_VERSION = "1.51.3s"; python build.py` | `1.51.3s` |
| 3 | **`git describe --tags --abbrev=0 --match 'v[0-9]*'`** | `git tag -a v1.51.3s -m "..."; python build.py` | always the bare base tag → `1.51.3s` (distance / dirty state never appear in the version string) |
...
All three scripts pick up `v1.51.3s` automatically (precedence #3 — Git finds the newest reachable annotated tag). This remains true when `HEAD` is a later untagged commit, because Tlamatini deliberately emits the bare base tag without a distance or dirty suffix.

You'll see this in each script's output:
```
Tlamatini version : 1.51.3s
...
The final artefact is **`dist/Tlamatini_Release_v1.51.3s/`** — zip and distribute.
...
- **About dialog**: `Tlamatini v1.51.3s`
- **Right-click `Tlamatini.exe` → Properties → Details**: ProductVersion = `1.51.3s`
...
TAG IT                git tag -a v1.51.3s -m "Release 1.51.3s"
                      git push origin v1.51.3s
...
WHERE IT LANDS        dist/Tlamatini_Release_v1.51.3s/
                      About dialog : "Tlamatini v1.51.3s"
                      Exe → Properties → Details : ProductVersion = 1.51.3s
                      curl /agent/version/ : {"version":"1.51.3s", …}
                      Console banner : --- [VERSION] Tlamatini 1.51.3s
