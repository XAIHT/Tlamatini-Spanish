...
2. **Single source of truth**: a **git tag** of the form `v1.50.0s`.
...
4. **Three injection points**, all computed automatically at build time:
   ...
   - The release folder name (`dist/Tlamatini_Release_v1.50.0s/`)
...
**Tlamatini-Spanish tags its releases with a trailing edition letter — `v1.50.0s` —
so a Spanish build is distinguishable at a glance from the English tree's `v1.50.0`.**
...
| Surface | Shows |
|---|---|
| About dialog, startup banner, `GET /agent/version/` | `1.50.0s` — the letter is kept |
| `.exe` → Properties → Details → ProductVersion (string) | `1.50.0s` — the letter is kept |
| Win32 `VERSIONINFO` numeric tuple (`filevers`/`prodvers`) | `(1, 50, 0, 0)` — the letter is stripped |
| Self-update's newer/older comparison | `1.50.0` — the letter is stripped |
...
purpose and returns `None` for `1.50.0s`.
...
reported ProductVersion 0.0.0.0 to Windows**; and self-update's comparison fell
through to a crude numeric split that **dropped the patch field**, so `1.49.1s`
compared equal to `1.49.0s` and **a patch release was never offered to a Spanish
user**. Both are invisible from the UI. Pinned by
`agent/test_edition_version_suffix.py`.
...
build.py / build_installer.py / build_uninstaller.py
   ...
   ├─►  dist/Tlamatini_Release_v1.50.0s/      (release folder name)
   └─►  Runtime surfaces:
            ├── About dialog:   "Tlamatini v1.50.0s"
            ├── Console banner: "--- [VERSION] Tlamatini 1.50.0s"
...
|---|---|---|---|
| 1 | **`--version X.Y.Z` CLI flag** | `python build.py --version 1.50.0s` | `1.50.0s` |
| 2 | **`$env:TLAMATINI_VERSION`** | `$env:TLAMATINI_VERSION = "1.50.0s"; python build.py` | `1.50.0s` |
| 3 | **`git describe --tags --abbrev=0 --match 'v[0-9]*'`** | `git tag -a v1.50.0s -m "..."; python build.py` | always the bare base tag → `1.50.0s` (distance / dirty state never appear in the version string) |
...
If `git status` shows changes: commit or stash them first. Building with a dirty tree is technically allowed, but the public version intentionally remains bare SemVer, so `git status` is the only reliable pre-build dirty-tree signal.
...
The commands below show the current `v1.50.0s` release as a concrete example. That tag already exists: **do not rerun these commands, move it, or recreate it**. Substitute the next deliberate version when preparing a future release.

```powershell
git tag -a v1.50.0s -m "Release 1.50.0s: <one-line summary>"
```
...
All three scripts pick up `v1.50.0s` automatically (precedence #3 — Git finds the newest reachable annotated tag). This remains true when `HEAD` is a later untagged commit, because Tlamatini deliberately emits the bare base tag without a distance or dirty suffix.

You'll see this in each script's output:
```
Tlamatini version : 1.50.0s
...
The final artefact is **`dist/Tlamatini_Release_v1.50.0s/`** — zip and distribute.
...
- **About dialog**: `Tlamatini v1.50.0s`
- **Right-click `Tlamatini.exe` → Properties → Details**: ProductVersion = `1.50.0s`
- **Console banner on startup**: `--- [VERSION] Tlamatini 1.50.0s`
- **`curl http://localhost:8000/agent/version/`** (after login or with anonymous access since it's open):
  ```json
  {"version":"1.50.0s","build":"1.50.0s","commit":"abc1234","date":"2026-08-23T12:00:00Z","source":"generated"}
  ```
...
TAG IT                git tag -a v1.50.0s -m "Release 1.50.0s"
                      git push origin v1.50.0s
...
WHERE IT LANDS        dist/Tlamatini_Release_v1.50.0s/
                      About dialog : "Tlamatini v1.50.0s"
                      Exe → Properties → Details : ProductVersion = 1.50.0s
                      curl /agent/version/ : {"version":"1.50.0s", …}
                      Console banner : --- [VERSION] Tlamatini 1.50.0s
